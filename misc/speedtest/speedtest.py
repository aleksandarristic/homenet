...
def run_speedtest(server_id="1301"):
    timestamp = datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')

    cmd = " ".join(["speedtest", "--accept-license", "--accept-gdpr", "-f", "json", "-s", server_id])
    stdout = ''
    ...
    return data, proc.returncode, timestamp
...
def do_test():
    ...
    raw_test_data, returncode, timestamp = run_speedtest()
    write_data(raw_test_data, timestamp)
    log.debug(f'Test done.')


def format_for_influxdb(raw_test_data, timestamp):
    if not raw_test_data:
        raw_test_data = {}
    data = {
...
def write_data(test_data, timestamp):
    ...
    data = format_for_influxdb(test_data, timestamp)
